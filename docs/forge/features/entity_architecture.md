...
`many_to_many` est un type de relation supporté dans `relations.json`. `sync:relations` génère la table pivot ; `make:crud` génère le formulaire côté source et l'affichage des libellés dans list/show. Voir [docs/relations.md](/docs/forge/features/relations/) pour la référence complète.
